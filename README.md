...
The following versions cannot be built as a Windows DLL (shared library) because they
predate an upstream fix ([nodejs/node#63963](https://github.com/nodejs/node/pull/63963)):

- **24.19.0**
- **>= 26.0.0 and < 26.7.0** (i.e. 26.0.0 – 26.6.x)

The fix landed upstream in Node.js **v26.7.0**. For these versions the Windows build
fails at link time: `node_mksnapshot` links `node_base` statically, but its symbols are
compiled as `dllimport` (via `BUILDING_NODE_EXTENSION`), which `lld-link` cannot resolve.
...
```bash
git tag v24.21.0
git push origin v24.21.0
```

Supported tag formats:
- `v22.23.3` → build Node.js 22.23.3
- `libnode-22.23.3` → same, alternative prefix

The workflow will build and create a GitHub Release attached to that tag.

### Option 2: Manual trigger (ad-hoc / testing)

Go to **Actions** → **Build libnode** → **Run workflow**, enter the version (e.g. `24.21.0`).

A release will be created with tag `libnode-v{version}` (e.g. `libnode-v24.21.0`).
